export async function getOpenAIEmbedding(texts: string[], apiKey?: string): Promise<number[][]> {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (probe && probe.status < 400) {
      ...
      if (!j || !Array.isArray(j.data)) throw new Error('Unexpected proxy response shape');
      const embeddings = j.data.map((d: { embedding: number[] }) => d.embedding);
      return embeddings;
    }
  }
  ...
  const j = await res.json();
  if (!j || !Array.isArray(j.data)) throw new Error('Unexpected OpenAI response shape');
  const embeddings = j.data.map((d: { embedding: number[] }) => d.embedding);
  return embeddings;
}
